...
Apply `migrations/0001_model_call_log.sql` (additive; five new tables, nothing altered) and set `SPORTLY_DATABASE_URL`. `server/store/pgDriver.ts` is the only file that names a driver, and it picks one by connection string:
