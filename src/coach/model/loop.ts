...
export function normalizeModelOutput(raw: unknown): CoachModelOutput {
  if (!raw || typeof raw !== 'object') throw new ProviderError('malformed', t('service.model.noOutput'))
  ...
  if (!message && !toolCalls.length) throw new ProviderError('malformed', t('service.model.noMessage'))
...
async function withTimeout<T>(run: (signal: AbortSignal) => Promise<T>, ms: number): Promise<T> {
  ...
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => {
      controller.abort()
      reject(new ProviderError('timeout', t('service.model.timeout', { s: Math.round(ms / 1000) })))
    }, ms)
  })
  try {
...
function summarise(records: ActionRecord[]): string {
  ...
  return done.length ? t('service.model.doneSoFar', { done: done.join('; ') }) : ''
}
...
export async function runModelTurn(provider: ModelProvider, conversationId: string | undefined, req: ModelTurnRequest, opts: ModelTurnOptions = {}): Promise<ModelTurnResult> {
  ...
  const finish = (output: Partial<CoachModelOutput>, limitReached: boolean, iterations: number): ModelTurnResult => {
    ...
    const text = output.message?.trim() || [summarise(records), limitReached ? t('service.model.stopped') : ''].filter(Boolean).join(' ') || t('service.model.nothingChanged')
    return {
  ...
  for (let i = 0; i < limits.maxIterations; i++) {
    ...
    opts.onStatus?.(i === 0 ? t('service.status.thinking') : t('service.status.checkingResult'))
    ...
    for (const call of output.toolCalls) {
      if (halted) {
        results.push({ toolCallId: call.id, name: call.name, ok: false, error: { code: 'skipped', message: t('service.model.skipped') }, affectedEntities: [], changes: [] })
        continue
      }
      if (calls >= limits.maxToolCalls) {
        results.push({ toolCallId: call.id, name: call.name, ok: false, error: { code: 'limit_reached', message: t('service.model.limit', { n: limits.maxToolCalls }) }, affectedEntities: [], changes: [] })
        halted = true
        continue
      }
      calls++
...
function statusFor(tool: string): string {
  if (tool.startsWith('get_') || tool === 'search_knowledge') return t('service.status.checkingData')
  if (tool.includes('workout') || tool === 'plan_week') return t('service.status.workout')
  if (tool.includes('program')) return t('service.status.program')
  if (tool.includes('meal') || tool.includes('nutrition')) return t('service.status.food')
  return t('service.status.applying')
}
